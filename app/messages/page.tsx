...
/*
 * Load messages one conversation at a time.
 *
 * Firestore security rules can then verify that the
 * logged-in user is a participant in each conversation.
 */
...
const messagesError = messageResults.find(
  (result) => result.error
)?.error;
...
const messageRows = messageResults.flatMap(
  (result) => result.data || []
);
