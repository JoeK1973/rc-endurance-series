...
(teamData || []).forEach((team: { id: string; name?: string }) => {
...
if (pError) {
    setNotice(`PROFILE SAVE ERROR: ${pError.message}`);
    setSaving(false);
    return;
}
...
setNotice(
    dError
        ? `DRIVER SAVE ERROR: ${dError.message}`
  ...
  async function linkGoogleAccount() {
    ...
    try {
      ...
      const { error } = await s.auth.linkGoogleAccount();

      if (error) {
        setNotice(`Could not connect Google: ${error.message}`);
        return;
      }

      setNotice(
        "Google account connected successfully. You can now use Google to sign in to this account."
      );
    } catch (error) {
      setNotice(
      ...
      {tab === "profile" && (
        <div className="space">
          ...
          >
            ...
            <label>
              ...
              <textarea
                ...
                value={driver.endurance_experience}
                onChange={(e) =>
                  setDriver({
                    ...driver,
                    endurance_experience:
          ...
          <div className="card space">
            ...
            <p className="muted">
              Your account currently uses email and password.
              You can connect a Google account so you can use
              Google to sign in to this same account.
            </p>
